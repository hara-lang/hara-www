...
The optional `modules/db-postgres` package is the reference stateful
module. Its identity is `gh:hara-lang:db-postgres`, its service export is
`db.postgres`, and its ABI is `hara.db-provider/1`; it is not a dependency
