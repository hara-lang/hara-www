...
test("the WWW navigation stays compact and left anchored through desktop", async () => {
  const shell = await read("src/styles/shell.css");

  assert.match(shell, /\.hara-www-secondary \{[\s\S]*?min-height: 48px;[\s\S]*?max-height: 48px;/);
  assert.match(shell, /\.hara-www-secondary__line \{[\s\S]*?display: flex;[\s\S]*?max-height: 48px;/);
  assert.match(shell, /\.hara-www-secondary__panel \{[\s\S]*?left: clamp\(12px, 2vw, 28px\);[\s\S]*?width: min\(340px, calc\(100vw - 56px\)\);/);
...
test("v2 adoption leaves homepage content full-width and clears both sticky rows", async () => {
  ...
  assert.match(bridge, /scroll-margin-top: calc\(var\(--hara-v2-header-height\) \+ 48px \+ 1rem\)/);
